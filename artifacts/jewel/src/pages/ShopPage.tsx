...
export default function ShopPage() {
  ...
  const toggleFilter = (type: 'category' | 'metal', value: string) => {
    if (type === 'category') {
      ...
      setSelectedMetals(prev => prev.includes(value) ? prev.filter(m => m !== value) : [...prev, value]);
    }
  };

  const clearFilters = () => { setSelectedCategories([]); setSelectedMetals([]); };
  ...
  if (sortOption === 'price-low') filteredProducts.sort((a, b) => (a.salePrice || a.price) - (b.salePrice || b.price));
  else if (sortOption === 'price-high') filteredProducts.sort((a, b) => (b.salePrice || b.price) - (a.salePrice || a.price));
  else if (sortOption === 'newest') filteredProducts.sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0));
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="space-y-3">
          {categories.map(cat => (
            <label key={cat} onClick={() => toggleFilter('category', cat)} className="flex items-center gap-3 cursor-pointer group">
              <motion.div
                animate={{ backgroundColor: selectedCategories.includes(cat) ? 'var(--foreground)' : 'transparent' }}
                className="w-4 h-4 border border-border flex items-center justify-center transition-colors group-hover:border-foreground"
              >
      ...
      <div>
        ...
        <div className="space-y-3">
          {metals.map(metal => (
            <label key={metal} onClick={() => toggleFilter('metal', metal)} className="flex items-center gap-3 cursor-pointer group">
              <motion.div
                animate={{ backgroundColor: selectedMetals.includes(metal) ? 'var(--foreground)' : 'transparent' }}
                className="w-4 h-4 border border-border flex items-center justify-center transition-colors group-hover:border-foreground"
              >
    ...
    <div className="pt-24 pb-24 min-h-screen">
      <div className="container mx-auto px-4 md:px-8">
        ...
        >
          <h1 className="font-serif text-4xl lg:text-5xl mb-4">
            {selectedCategories.length === 1
              ? <span className="capitalize">{selectedCategories[0]}</span>
              : 'All Fine Jewelry'}
          </h1>
          <p className="text-muted-foreground">Explore our collection of solid gold, platinum, and sterling silver pieces.</p>
        </motion.div>
        ...
        <motion.div
          ...
          className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-y border-border py-4"
        >
        ...
        <AnimatePresence>
          {activeFilterCount > 0 && (
            ...
            >
              ...
              <button onClick={clearFilters} className="text-xs text-muted-foreground underline ml-2 hover:text-foreground">
                Clear All
              </button>
            </motion.div>
          )}
            ...
            <AnimatePresence mode="wait">
              {filteredProducts.length > 0 ? (
                ...
                >
                  ...
                  <button onClick={clearFilters} className="bg-foreground text-background uppercase tracking-[0.12em] text-[11px] px-8 py-3 font-medium hover:opacity-90 transition-opacity">
                    Clear Filters
                  </button>
                </motion.div>
              )}
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-border">
                ...
                <button onClick={() => setIsMobileFilterOpen(false)} className="p-2"><X size={20} strokeWidth={1.5} /></button>
              </div>
              <div className="flex-1 overflow-y-auto p-6"><FilterContent /></div>
              <div className="p-4 border-t border-border flex gap-4">
                <button onClick={clearFilters} className="flex-1 py-4 border border-border text-[11px] uppercase tracking-[0.12em] font-medium">Clear</button>
                <button onClick={() => setIsMobileFilterOpen(false)} className="flex-1 bg-foreground text-background py-4 text-[11px] uppercase tracking-[0.12em] font-medium">Apply</button>
              </div>
            </motion.div>
          </>
        )}
